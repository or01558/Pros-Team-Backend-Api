...
function createClientNonce(length = 32) {
    return platform_1.randomBytes(length).toString('hex');
}
...
function H(text, alg) {
    return platform_1.createHash(alg).update(text).digest();
}
exports.H = H;
function HMAC(key, msg, alg) {
    return platform_1.createHmac(alg, key).update(msg).digest();
}
exports.HMAC = HMAC;
...
class DIGEST extends SimpleMech {
    ...
    createResponse(credentials) {
        ...
        str += ',qop=' + qop;
        str += ',digest-uri="' + uri + '"';
        const base = platform_1.createHash('md5')
            ...
            .digest();
        const ha1 = platform_1.createHash('md5')
            .update(base)
            .update(':')
            .update(this.nonce)
            .update(':')
        ...
        const ha2 = platform_1.createHash('md5').update('AUTHENTICATE:').update(uri);
        const dha2 = ha2.digest('hex');
        const digest = platform_1.createHash('md5')
            .update(dha1)
            .update(':')
            .update(this.nonce)
            .update(':')
...
class SCRAM {
    ...
    challengeResponse(credentials) {
        ...
        if (cached && credentials.clientKey && credentials.serverKey) {
            ...
            saltedPassword = Hi(platform_1.Buffer.from(stringprep_1.saslprep(credentials.password)), this.salt, this.iterationCount, this.algorithm);
            clientKey = HMAC(saltedPassword, CLIENT_KEY, this.algorithm);
            serverKey = HMAC(saltedPassword, SERVER_KEY, this.algorithm);
        }
